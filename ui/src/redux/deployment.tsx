...
>('deployment/fetchDeployment', async (_, { getState, rejectWithValue }) => {
...
export const fetchDeploymentChanges = createAsyncThunk<
  Commit[],
  void,
  { state: { deployment: DeploymentState } }
>(
...
export const deployToSCM = createAsyncThunk<
  Deployment,
  void,
  { state: { deployment: DeploymentState } }
>(
  ...
  async (_, { getState, rejectWithValue, requestId }) => {
    const { namespace, name, number, deploying, deployId } =
      getState().deployment;
...
export const fetchReviews = createAsyncThunk<
  ...
  void,
  { state: { deployment: DeploymentState } }
>('deployment/fetchReviews', async (_, { getState, rejectWithValue }) => {
  const { namespace, name, number } = getState().deployment;
...
export const approve = createAsyncThunk<
  ...
  string,
  { state: { deployment: DeploymentState } }
>('deployment/approve', async (comment, { getState, rejectWithValue }) => {
  const { namespace, name, number } = getState().deployment;
...
export const reject = createAsyncThunk<
  ...
  string,
  { state: { deployment: DeploymentState } }
>('deployment/reject', async (comment, { getState, rejectWithValue }) => {
  const { namespace, name, number } = getState().deployment;
...
export const fetchUserReview = createAsyncThunk<
  ...
  void,
  { state: { deployment: DeploymentState } }
>('deployment/fetchUserReview', async (_, { getState, rejectWithValue }) => {
  const { namespace, name, number } = getState().deployment;
...
export const deploymentSlice = createSlice({
  ...
  reducers: {
    init: (
      state,
      action: PayloadAction<{ namespace: string; name: string; number: number }>
    ) => {
