...
export default function RollbackForm(props: RollbackFormProps): JSX.Element {
  ...
  const onSelectDeployment = (id: number) => {
    ...
    if (deployment === undefined)
      throw new Error("The deployment doesn't exist.");
  ...
  return (
    <Form name="rollback" onFinish={onFinish}>
      <Form.Item
        label="Environment"
        name="environment"
        {...selectLayout}
            ...
            if (d.deployer) {
              option = (
                <Select.Option key={idx} value={d.id}>
                  #{d.number} - <DeploymentRefCode deployment={d} /> deployed by{' '}
                  <Avatar size="small" src={d.deployer.avatar} />{' '}
                  {d.deployer.login} {moment(d.createdAt).fromNow()}
                </Select.Option>
              );
            } else {
              option = (
                <Select.Option key={idx} value={d.id}>
                  #{d.number} - <DeploymentRefCode deployment={d} /> deployed{' '}
