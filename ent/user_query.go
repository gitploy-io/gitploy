...
func (uq *UserQuery) QueryChatUser() *ChatUserQuery {
	query := &ChatUserQuery{config: uq.config}
...
func (uq *UserQuery) QueryPerms() *PermQuery {
	query := &PermQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withChatUser; query != nil {
		...
		query.Where(predicate.ChatUser(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ChatUserColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.ChatUser = n
		}
	}

	if query := uq.withPerms; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
	...
	if query := uq.withNotification; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Notification = []*Notification{}
		}
		query.Where(predicate.Notification(func(s *sql.Selector) {
			s.Where(sql.InValues(user.NotificationColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Notification = append(node.Edges.Notification, n)
		}
	}
